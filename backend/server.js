...
app.get('/api/ranking', async function(req, res) {
  try {
    ...
    res.status(500).json({ error: '랭킹 조회 실패: ' + err.message });
  }
});
...
app.post('/api/ranking', async function(req, res) {
  try {
    ...
    res.status(500).json({ error: '랭킹 저장 실패: ' + err.message });
  }
});
...
app.post('/api/coach', async function(req, res) {
  ...
  if (!mode) return res.status(400).json({ error: '필수 파라미터가 없습니다.' });
  ...
  if (mode === 'feedback' && !question) return res.status(400).json({ error: '질문을 입력해주세요.' });
  if (mode === 'generate' && !topic) return res.status(400).json({ error: '주제를 입력해주세요.' });
  ...
  try {
    ...
    var resultData = JSON.parse(clean);
